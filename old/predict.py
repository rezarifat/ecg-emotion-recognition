...
FS_MODEL = 256        # what our model was trained on
WINDOW   = FS_MODEL * 10  # 2560 samples
...
def load_dicom(filepath):
    ...
    try:
        ...
        try:
            fs = float(ds[0x003a, 0x001a].value)  # WaveformSampleInterpreter
        except:
...
def predict_from_file(filepath: str):
    ...
    # Use middle 10s window — most stable
    mid   = len(ecg_clean) // 2
